...
1. `seed` (String) Input keying material (raw bytes). Any length — HKDF stretches to the 48 bytes the scalar derivation needs. Must not be empty and must not exceed 8388608 bytes (8 MiB). For cryptographic security pass at least 16 bytes of high-entropy input.
