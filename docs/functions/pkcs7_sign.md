...
description: |-
  CMS / PKCS#7 sign bytes with an ECDSA P-256 identity (deterministic; RFC 5652 §5.3 no-signed-attrs shape)
---

# function: pkcs7_sign

Produces a CMS SignedData ContentInfo (RFC 5652) carrying `data` as its encapsulated content. The signer cert is embedded; the signature is RFC 6979 deterministic ECDSA-with-SHA256 over the raw content; no signed attributes are included (RFC 5652 §5.3 permits omitting `signedAttrs` when the encapsulated content type is `id-data`).
