---
# generated by https://github.com/hashicorp/terraform-plugin-docs
# subcategory written by cmd/gendoctemplates; do not edit by hand
page_title: "geohash_decode function - burnham"
subcategory: "Geographic"
description: |-
  Decode a geohash into the centre point and bounding box of its cell
---

<!--
  DO NOT EDIT. This page is generated by "go generate ./..." and any change here is overwritten on the next run.
  Edit the source instead:
    prose (MarkdownDescription): internal/provider/geographic/descriptions/geohash_decode.md
    example (Example Usage):     examples/functions/geohash_decode/function.tf
-->

# function: geohash_decode

<!-- Edit here: this is the MarkdownDescription source for the burnham geohash_decode function. docs/functions/geohash_decode.md is generated from it by "go generate ./..."; do not edit the generated doc. -->

Parses `code` and returns:

- `latitude` / `longitude`: the centre of the cell, in degrees.
- `lat_min` / `lat_max` / `lon_min` / `lon_max`: the cell's bounding box (the points the code *might* have been encoded from).

`code` is case-insensitive but must use the standard geohash alphabet: digits `0-9` plus lowercase `b-z` with `i`, `l`, `o` removed. Errors on any other character.

The upstream bounding box reaches exactly `90` on the northern edge and exactly `180` on the eastern edge (the corner cell `zzz…z` hits both). Those values wrap if fed back into the encoder, so `geohash_decode` instead reports `lat_max` / `lon_max` a few ULPs below the geometric edge (`90` / `180`), a value the encoder still accepts, keeping the cell round-trippable back through `geohash_encode`.

## Example Usage

```terraform
// Decode a geohash into the centre point and bounding box of its cell.
output "civic_center_decoded" {
  value = provider::burnham::geohash_decode("9q8yyk8")
  // → { latitude = 37.7749…, longitude = -122.4194…, lat_min = …, lat_max = …, lon_min = …, lon_max = … }
}

// Round-trip a coordinate through encode + decode and check the centre lands inside the cell.
output "is_centre_inside_cell" {
  value = (
    provider::burnham::geohash_decode(provider::burnham::geohash_encode(37.7749, -122.4194, 7)).lat_min < 37.7749 &&
    provider::burnham::geohash_decode(provider::burnham::geohash_encode(37.7749, -122.4194, 7)).lat_max > 37.7749
  )
}
```

## Signature

<!-- signature generated by tfplugindocs -->
```text
geohash_decode(code string) object
```

## Arguments

<!-- arguments generated by tfplugindocs -->
1. `code` (String) Geohash string to decode.
