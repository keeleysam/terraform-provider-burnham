---
# generated by https://github.com/hashicorp/terraform-plugin-docs
# subcategory written by cmd/gendoctemplates; do not edit by hand
page_title: "ip_to_mixed_notation function - burnham"
subcategory: "Networking"
description: |-
  Format an IPv6 address using mixed (dual) x:x:x:x:x:x:d.d.d.d notation
---

<!--
  DO NOT EDIT. This page is generated by "go generate ./..." and any change here is overwritten on the next run.
  Edit the source instead:
    prose (MarkdownDescription): internal/provider/network/descriptions/ip_to_mixed_notation.md
    example (Example Usage):     examples/functions/ip_to_mixed_notation/function.tf
-->

# function: ip_to_mixed_notation

<!-- Edit here: this is the MarkdownDescription source for the burnham ip_to_mixed_notation function. docs/functions/ip_to_mixed_notation.md is generated from it by "go generate ./..."; do not edit the generated doc. -->

Returns the IPv6 address formatted with the last 32 bits expressed as a dotted-decimal IPv4 address, e.g. `64:ff9b::192.0.2.1` instead of `64:ff9b::c000:201`. Zero-compression (::) is applied to the hex portion. IPv4 addresses are returned unchanged.

Common uses:

- Making NAT64 addresses human-readable in outputs and documentation.
- Expressing IPv4-compatible addresses in a form that makes the embedded IPv4 immediately visible.

## Example Usage

```terraform
// Format an IPv6 address with the last 32 bits in dotted-decimal, making embedded IPv4 visible at a glance.
output "human_readable" {
  value = provider::burnham::ip_to_mixed_notation("64:ff9b::c000:201")
  // → "64:ff9b::192.0.2.1"
}
```

## Signature

<!-- signature generated by tfplugindocs -->
```text
ip_to_mixed_notation(ip string) string
```

## Arguments

<!-- arguments generated by tfplugindocs -->
1. `ip` (String) An IPv6 (or IPv4) address string.
