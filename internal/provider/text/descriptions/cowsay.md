...
- `action` (string): `"say"` (default; a single-line bubble uses `< >`, a multi-line bubble uses `/ \` corners and `| |` sides, and the cow is drawn with a `\` connector) or `"think"` (uses `( )` throughout with `o` connectors).
