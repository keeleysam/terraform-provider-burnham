/*
Approximate-pi (22/7) digit functions (RFC 3091 §1.1 TCP and §2.2 UDP
...
22/7 = 3.142857142857… is a period-6 repeating decimal: long division of
22 by 7 yields the cycle "142857" starting after the decimal point. So
the n-th digit is "142857"[(n-1) mod 6] — constant-time lookup, no big
math required for the lookup itself, and no upper bound on n.

The single-digit function uses NumberParameter (arbitrary precision) so
truly enormous n (up to ~10^150 in Terraform's 512-bit number type) is
supported. The bulk function uses Int64Parameter for `count` because the
returned string must be materialized in memory; we don't try to handle
counts larger than int can address.
*/
...
// approximateDigitChar returns the n-th decimal digit of 22/7 *following*
// the decimal point. n must be >= 1; out-of-range is the caller's job.
//
...
// approximateFirstNDigits returns the first n digits of 22/7 *following*
// the decimal point. n must be in [0, math.MaxInt].
...
func (f *PiApproximateDigitFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns the n-th decimal digit of 22/7 *following* the decimal point, formatted as the " +
			"[RFC 3091 §2.1.2](https://www.rfc-editor.org/rfc/rfc3091#section-2.1.2) UDP reply payload " +
			"`reply = nth_digit \":\" DIGIT`. This is the \"approximate service\" of RFC 3091 §1.1/§2.2 — long division " +
			"of 22 by 7 gives `3.142857142857…`, a period-6 repeating cycle of `\"142857\"`.\n\n" +
			"Examples:\n" +
			"- `pi_approximate_digit(1)` → `\"1:1\"`\n" +
			"- `pi_approximate_digit(7)` → `\"7:1\"` (cycle wraps to start of `\"142857\"`)\n" +
			"- `pi_approximate_digit(100)` → `\"100:8\"`\n\n" +
			"**No upper bound on n.** Because 22/7 cycles with period 6, the n-th digit is just " +
			"`\"142857\"[(n-1) mod 6]` — a constant-time lookup. n can be arbitrarily large (up to ~10^150 " +
			"in Terraform's 512-bit number type) and the function returns instantly.",
...
func (f *PiApproximateDigitFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
	...
	// Defensive: (*big.Float).Int returns nil for ±Inf, which would panic
	// on the Sign()/String() calls below. Terraform's number parser
	// shouldn't produce Inf, but check explicitly so the contract holds
	// even if the framework changes.
	if n.IsInf() {
...
func (f *PiApproximateDigitsFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns the first `count` decimal digits of 22/7 *following* the decimal point as a single ASCII string. " +
			"Models the [RFC 3091 §1.1](https://www.rfc-editor.org/rfc/rfc3091#section-1.1) TCP approximate service, which " +
			"streams `\"starting with the most significant digit following the decimal point\"` — no seek operation, so this " +
			"function takes only `count`.\n\n" +
			"Example:\n" +
			"- `pi_approximate_digits(12)` → `\"142857142857\"` (the 6-digit cycle, twice)\n\n" +
			"Because 22/7 is a period-6 repeating decimal, output for any count `c` is just " +
			"`\"142857\"` repeated and truncated. Count is bounded only by what Go's `int` and your machine's memory can hold.",
		Parameters: []function.Parameter{
