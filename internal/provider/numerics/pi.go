...
Both are backed by the DPD-packed 3,141,592-digit (= ⌊π × 10⁶⌋) constant
in pi_data.go; no runtime computation. Out-of-range requests error
explicitly per RFC's implicit "no wrong digits" stance
...
// piMaxDigits is the upper bound on n for pi_digit and on count for
// pi_digits. RFC 3091 imposes no upper bound; this is an implementation
// cap matching the embedded packed digit count.
const piMaxDigits = piEmbeddedDigitCount

// piCapErrorMessage produces the RFC-aware error message we return when a
// caller asks for more digits than this implementation can produce.
// `paramName` is the function's parameter name ("n" for pi_digit, "count"
// for pi_digits) so the message refers to the actual argument.
func piCapErrorMessage(paramName string, received int64) string {
	return fmt.Sprintf(
		"this implementation supports %s up to %d (= floor(π × 10^6)); received %d. "+
			"RFC 3091 imposes no upper bound on the digit index, but materializing "+
			"more than ~π million decimal digits at plan time would meaningfully slow Terraform.",
...
func (f *PiDigitFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns the n-th decimal digit of π *following* the decimal point, formatted as the " +
			"[RFC 3091 §2.1.2](https://www.rfc-editor.org/rfc/rfc3091#section-2.1.2) UDP reply payload " +
			"`reply = nth_digit \":\" DIGIT`. No whitespace, no newline, leading 3 implied per the RFC's \"Note\" section.\n\n" +
			"Examples:\n" +
			"- `pi_digit(1)` → `\"1:1\"` (the first digit of π after the decimal is 1)\n" +
			"- `pi_digit(100)` → `\"100:9\"`\n\n" +
			"**Indexing.** `n` is 1-indexed per the RFC; `n < 1` errors.\n\n" +
			"**Implementation cap.** This function is backed by an embedded table of the first " +
			"3,141,592 = ⌊π × 10⁶⌋ digits of π, encoded as IEEE 754-2008 [Densely Packed Decimal](https://en.wikipedia.org/wiki/Densely_packed_decimal) " +
			"(3 digits per 10 bits, ≈3.33 bits/digit). Constant-time lookup, no plan-time computation. " +
			"`n` > 3,141,592 errors. RFC 3091 imposes no upper bound, but a Terraform provider that " +
			"shipped more digits would either bloat the binary or silently compute on every plan; " +
			"this implementation does neither.\n\n" +
			"**Floor, not round.** The cap is `floor(π × 10⁶) = 3,141,592` — *not* `round(π × 10⁶) = 3,141,593`. " +
			"Rounding up would require shipping a digit we haven't actually computed and verified; " +
			"[RFC 3091 §5](https://www.rfc-editor.org/rfc/rfc3091#section-5) is unequivocal that returning " +
			"incorrect digits is unacceptable: \"*The imminent collapse of the Internet is assured if this " +
			"guideline is not strictly followed.*\" So we floor.",
...
func (f *PiDigitsFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns the first `count` decimal digits of π *following* the decimal point as a single ASCII string. " +
			"Models the [RFC 3091 §1](https://www.rfc-editor.org/rfc/rfc3091#section-1) TCP service, which always streams " +
			"\"starting with the most significant digit following the decimal point\" — there is no seek operation in the protocol, " +
			"so this function takes only `count`, not a starting position.\n\n" +
			"Example:\n" +
