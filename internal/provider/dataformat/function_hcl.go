...
func (f *HCLDecodeFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Parses an arbitrary [HCL2](https://github.com/hashicorp/hcl) document — a sequence of `key = value` attribute statements — and returns it as a Terraform object. " +
			"Values are evaluated as static literals: numbers, strings, booleans, lists/tuples, and objects/maps work; references to variables, data sources, or function calls do **not** (there is no eval context).\n\n" +
...
func (f *HCLEncodeFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Encodes a Terraform object as a sequence of HCL attribute statements (`key = value` lines), one per object member, in alphabetical key order. " +
			"Nested objects render as HCL object literals (`{ ... }`); lists render as bracketed sequences; primitives render as their natural HCL representation.\n\n" +
