...
func (f *PlistDateFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns a tagged object representing an [`NSDate`](https://developer.apple.com/documentation/foundation/nsdate) plist value. " +
			"When passed to `plistencode`, this produces a `<date>` XML element with the given timestamp. `plistdecode` returns the same tagged-object " +
			"shape for `<date>` elements, so encode/decode round-trips preserve the type.\n\n" +
...
func (f *PlistDataFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns a tagged object representing an [`NSData`](https://developer.apple.com/documentation/foundation/nsdata) plist value. " +
			"When passed to `plistencode`, this produces a `<data>` XML element with the given binary payload. `plistdecode` returns the same " +
			"tagged-object shape for `<data>` elements, so encode/decode round-trips preserve the type.\n\n" +
			"The input must be a base64-encoded string. Pair with `filebase64(\"path/to/file\")` to embed file contents like certificates, " +
			"profile-signing material, or images.\n\n" +
...
func (f *PlistRealFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Returns a tagged object representing a plist `<real>` (floating-point) value. `plistencode` would normally encode " +
			"whole numbers as `<integer>` and only fractional numbers as `<real>` — this helper forces a whole number into `<real>` form when the " +
			"consumer expects a floating-point type. `plistdecode` returns the same tagged-object shape for whole-number `<real>` elements, preserving " +
			"the type across round-trips.\n\n" +
			"Fractional values like `3.14` already encode as `<real>` automatically — this helper is only needed for whole-number reals.\n\n" +
			"**Common uses:** profile fields that demand a floating-point type even when the value happens to be a whole number (e.g. some " +
			"`Rating`, `Score`, or version fields in MDM payloads).",
		Parameters: []function.Parameter{
