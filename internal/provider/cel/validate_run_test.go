...
// that celvalidate's description promises: on input past the size guard it must
...
func TestValidateRun_OversizeReturnsFalse(t *testing.T) {
	big := strings.Repeat("a", celMaxInputBytes+1)
	f := &CELValidateFunction{}
	emptyOpts := types.TupleValueMust([]attr.Type{}, []attr.Value{})
	args := function.NewArgumentsData([]attr.Value{types.StringValue(big), emptyOpts})
