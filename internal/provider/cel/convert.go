...
const (
	// celMaxDepth caps recursion when converting the input tree.
	// Real expressions nest a few dozen levels at most; this guards against adversarial input.
	celMaxDepth = 1024
...
// errUnknownValue signals that a value in the input tree is unknown at plan time.
// A plan-time function returns an unknown result in that case rather than failing, so the value can resolve at apply.
var errUnknownValue = errors.New("value is unknown")

// hasUnknown reports whether v holds an unknown value at any depth.
// Terraform only auto-defers a function call when a whole argument is unknown, so a known container with an unknown nested value reaches Run; the functions check this and return an unknown result rather than erroring, so the value resolves at apply.
...
// terraformToNode converts a Terraform attr.Value into the JSON-ish node tree the encoder consumes: nil, bool, string, json.Number, []any, map[string]any.
// Numbers stay json.Number so the encoder can distinguish int from double.
...
func terraformToNodeImpl(v attr.Value, depth int, nodes *int) (any, error) {
	if depth >= celMaxDepth {
		return nil, fmt.Errorf("input exceeds maximum supported nesting depth of %d", celMaxDepth)
	}
	*nodes++
	if *nodes > celMaxNodes {
		return nil, fmt.Errorf("input exceeds maximum supported node count of %d", celMaxNodes)
	}
	if v == nil || v.IsNull() {
		return nil, nil
	}
