...
func (f *JSONPathQueryFunction) Definition(_ context.Context, _ function.DefinitionRequest, resp *function.DefinitionResponse) {
	resp.Definition = function.Definition{
		...
		MarkdownDescription: "Evaluates an [RFC 9535](https://www.rfc-editor.org/rfc/rfc9535.html) JSONPath expression against a Terraform value and returns the list of matching nodes. " +
			"Useful for extracting subsets of nested structures using the standardized JSONPath grammar.\n\n" +
			"The expression must conform to RFC 9535 — the IETF standardized JSONPath. Common selectors include the root identifier (`$`), " +
			"name selectors (`$.store.book`), wildcard (`$..*`), descendant segments (`$..price`), array slices (`$[0:5]`), and filters (`$[?@.price < 10]`).\n\n" +
			"Returns a list of matching values. An expression that matches nothing returns an empty list. To collapse single-match queries to a scalar, " +
			"use `one(provider::burnham::jsonpath_query(...))` or index the first element.\n\n" +
