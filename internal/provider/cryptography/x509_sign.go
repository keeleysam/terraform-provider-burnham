...
func (f *X509SelfSignFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
	...
	// RFC 5280 §A.1 sets ub-common-name-length := 64, which is a 64-*character* cap (ASN.1 CHARACTER STRING), not 64 bytes. Multi-byte UTF-8 input would be rejected by a byte-based check even when RFC-legal, so we count runes.
