...
Ed25519 is naturally deterministic by spec (RFC 8032 §5.1.6 — the signing scalar and nonce are derived from the private-key seed plus the message), so unlike the ECDSA path there's no RFC 6979 wrapper needed. The `ed25519.PrivateKey` returned by `ed25519.NewKeyFromSeed` already satisfies `crypto.Signer` with deterministic behaviour, and both `x509.CreateCertificate` and `digitorus/pkcs7.SignWithoutAttr` produce byte-stable output when handed it directly.
*/
