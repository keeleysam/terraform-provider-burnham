...
Builds a SignedData (RFC 5652) ContentInfo: encapsulated `id-data`, no signed attributes (RFC 5652 §5.3 — `signedAttrs` is OPTIONAL when the content type is `id-data`), embedded signer cert, signature over the raw content. The output is byte-identical across runs for the same inputs.
...
The no-signed-attributes choice matches the wire format Apple's configuration-profile installer accepts on macOS — and avoids the `signingTime: time.Now()` attribute that would re-introduce non-determinism. CMS signing libraries that always add signed attributes can't produce this shape; we use [`github.com/digitorus/pkcs7`](https://github.com/digitorus/pkcs7)'s `SignWithoutAttr` for it. Apple's macOS profile installer rejects Ed25519-signed mobileconfigs at the keychain-import layer as of macOS 26.5 — use ECDSA P-256 for that workflow; Ed25519 is for everything else (OpenSSL `cms`, container signing, internal tooling).
