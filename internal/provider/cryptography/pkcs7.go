/*
`pkcs7_sign`: CMS/PKCS#7 sign arbitrary bytes with an ECDSA P-256 or Ed25519 identity, deterministically.

Builds a SignedData (RFC 5652) ContentInfo: encapsulated `id-data`, no signed attributes (RFC 5652 §5.3 makes `signedAttrs` OPTIONAL when the content type is `id-data`), embedded signer cert, signature over the raw content. The output is byte-identical across runs for the same inputs.

  - **ECDSA P-256**: signature is RFC 6979 deterministic ECDSA-with-SHA256.
  - **Ed25519**: signature is PureEdDSA (RFC 8419 §3.1, message signed directly, no pre-hash). SHA-512 is still set as the SignerInfo's `digestAlgorithm` per RFC 8419 §3 even though it's not used in the signature math itself.

The no-signed-attributes choice matches the wire format Apple's configuration-profile installer accepts on macOS, and avoids the `signingTime: time.Now()` attribute that would re-introduce non-determinism. CMS signing libraries that always add signed attributes can't produce this shape; we use [`github.com/digitorus/pkcs7`](https://github.com/digitorus/pkcs7)'s `SignWithoutAttr` for it. Apple's macOS profile installer rejects Ed25519-signed mobileconfigs at the keychain-import layer as of macOS 26.5, so use ECDSA P-256 for that workflow; Ed25519 is for everything else (OpenSSL `cms`, container signing, internal tooling).
...
func (f *PKCS7SignFunction) Run(ctx context.Context, req function.RunRequest, resp *function.RunResponse) {
	...
	// Without this check, mismatched key/cert silently produces a CMS that no verifier can validate: a class of late-stage failure (rejected at install time) that's cheap to catch here. Both `*ecdsa.PublicKey` and `ed25519.PublicKey` implement `Equal(crypto.PublicKey) bool`, so the assertion-then-Equal path covers both algorithms without algorithm-specific branching here.
	type publicKeyEqualer interface {
		Equal(x crypto.PublicKey) bool
	}
	pub, ok := signer.Public().(publicKeyEqualer)
	if !ok {
		// Shouldn't happen: parseSigningPrivateKey only returns ECDSA-P256 or Ed25519, and both implement Equal. Belt-and-braces in case a future code path slips in a key type without Equal.
		resp.Error = function.NewFuncError(fmt.Sprintf("internal: signer public key %T does not implement Equal", signer.Public()))
		return
	}
	...
	// Choose the SignerInfo `digestAlgorithm` based on key type. For ECDSA P-256 we sign SHA-256(data); for Ed25519 (PureEdDSA) the signature is over raw data with no pre-hashing, but RFC 8419 §3 still requires us to emit SHA-512 in the SignedData `digestAlgorithms` SET / SignerInfo's `digestAlgorithm`.
	var digestOID asn1.ObjectIdentifier
	switch signer.Public().(type) {
	case *ecdsa.PublicKey:
		digestOID = pkcs7.OIDDigestAlgorithmSHA256
	case ed25519.PublicKey:
		digestOID = pkcs7.OIDDigestAlgorithmSHA512
	default:
		resp.Error = function.NewFuncError(fmt.Sprintf("internal: unsupported signer public-key type %T", signer.Public()))
		return
	}
