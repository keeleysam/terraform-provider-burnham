...
func (p *BurnhamProvider) Schema(_ context.Context, _ provider.SchemaRequest, resp *provider.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description: "A pure provider-defined function provider for structured data formats, IP/CIDR operations, query/patch transformations over decoded values, and a small numerics family (RFC 3091). No resources, no data sources, no remote API calls — every function evaluates at plan time.",
	}
}
