...
The default alphabet is the 64-character URL-safe set `_-0-9A-Za-z` (the same URL-safe character set the upstream [nanoid](https://github.com/ai/nanoid) reference uses, though in sorted order rather than upstream's scrambled default, so the IDs differ) and the default `size` is 21 characters. Both can be overridden via the optional `options` object:
