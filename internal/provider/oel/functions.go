// Package oel provides Terraform provider functions to build, validate, format, decode, and evaluate Okta Expression Language (OEL) expressions from HCL data.
//
...
// Functions returns the Okta Expression Language provider functions registered by terraform-burnham: oelencode (build an OEL string from HCL data), oelvalidate (report whether a string is valid OEL), oelformat (canonicalize an OEL string), oeldecode (parse an OEL string back into the data tree), and oelevaluate (evaluate an OEL expression against a sample context).
